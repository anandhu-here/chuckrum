Chuckrum Core version 0.11.0 is now available from:

  <https://chuckrum.org/bin/chuckrum-core-0.11.0/>
...
For example, add the following to `chuckrum.conf`:
...
related to the blockchain in the chuckrum system: the raw blocks as received over 
the network (blk???.dat), the undo data (rev???.dat), the block index and the 
UTXO set (both LevelDB databases). The databases are built from the raw data.

Block pruning allows Chuckrum Core to delete the raw block and undo data once 
...
block index and UTXO databases. The minimum was chosen so that Chuckrum Core will 
...
For an example script see [chuckrum-submittx](https://github.com/laanwj/chuckrum-submittx).
...
- #5334 `cf87536` libchuckrumconsensus: Add pkg-config support
...
- #6269 `95aca44` gitian: Use the new chuckrum-detached-sigs git repo for OSX signatures
...
- #5729 `9c4a5a5` Allow unit changes for read-only ChuckrumAmountField
- #5753 `0f44672` Add chuckrum logo to about screen
...
- #5809 `46bfbe7` Add chuckrum-cli man page
...
- #5849 `41259ca` contrib/init/chuckrumd.openrc: Compatibility with previous OpenRC init script variables
...
- #6116 `30dc3c1` [OSX] rename Chuckrum-Qt.app to Chuckrum-Core.app
- #6168 `b3024f0` contrib/linearize: Support linearization of testnet blocks
- #6098 `7708fcd` Update Windows resource files (and add one for chuckrum-tx)
