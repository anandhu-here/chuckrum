...
###   chuckrumcore.org and chuckrum.org and compares them.
...
WORKINGDIR="/tmp/chuckrum_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://chuckrumcore.org"
HOST2="https://chuckrum.org"
BASEDIR="/bin/"
VERSIONPREFIX="chuckrum-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'chuckrum-',
...
if [ $? -ne 0 ]; then
   echo "chuckrum.org failed to provide signature file, but chuckrumcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "chuckrum.org and chuckrumcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Chuckrum Core binary release signing key installed?"
   fi
