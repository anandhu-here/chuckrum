...
const char * const CHUCKRUM_CONF_FILENAME = "chuckrum.conf";
const char * const CHUCKRUM_PID_FILENAME = "chuckrumd.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "chuckrumd -foo=bar
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Chuckrum
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Chuckrum
    // Mac: ~/Library/Application Support/Chuckrum
    // Unix: ~/.chuckrum
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Chuckrum";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Chuckrum";
#else
    // Unix
    return pathRet / ".chuckrum";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", CHUCKRUM_CONF_FILENAME);
...
fs::path GetPidFile()
{
    return AbsPathForConfigVal(fs::path(gArgs.GetArg("-pid", CHUCKRUM_PID_FILENAME)));
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Chuckrum Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Chuckrum Core") == std::string::npos) {
